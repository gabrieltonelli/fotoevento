...
-- =============================================
-- PROFILES TABLE (User account status)
-- =============================================
CREATE TABLE IF NOT EXISTS profiles (
  id UUID PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
  full_name TEXT,
  subscription_plan TEXT DEFAULT 'none' CHECK (subscription_plan IN ('none', 'free', 'pro', 'premium')),
...
RETURNS TRIGGER AS $$
BEGIN
  INSERT INTO public.profiles (id, full_name, subscription_plan, subscription_status)
  VALUES (NEW.id, NEW.raw_user_meta_data->>'full_name', 'none', 'inactive');
  RETURN NEW;
END;
